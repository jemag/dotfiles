...
markQueue() {
  ...
  queue=()
  j=0
  for i in "${queueCopy[@]}"; do
    swaymsg "unmark _prevFocus$j" 1>/dev/null 2>&1
    if swaymsg "[id=$i] mark --add _prevFocus$j" 2>/dev/null | grep -i "\"success\":true" 1>/dev/null 2>&1; then
      queue=(${queue[@]:0} "$i")
      j=$(($j + 1))
    fi
  done
...
xprop -root -spy _NET_ACTIVE_WINDOW |
  while read line; do
    ...
    if [ $period -gt $permanence -o "$currentFocus" = "${queue[0]}" ]; then
